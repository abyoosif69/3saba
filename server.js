...
setInterval(() => {
  http.get(`http://autonitro.glitch.me/`);
}, 280000);

console.log("Nitro For Free");

bot.on("ready", () => {
  console.log(
    `Logged in as: ${chalk.yellow(bot.user.tag)}\nEmail: ${chalk.bold(
      bot.user.email
    )}\nID: ${chalk.bold(bot.user.id)}`
  );
  title(
    `${bot.user.tag} | ${bot.guilds.size} guilds | ${bot.user.friends.size} friends`
  );
  if (config.presence !== true) return;

  try {
    const client = require("discord-rich-presence")("649290475492212744");
  ...
  if (msg.content.toLowerCase().startsWith("$blacklist")) {
    if (!args[0] || !args[1])
      return msg.channel.send(
        `Please use the command like this: \`$blacklist add/remove guildid/userid/channelid\`. This will add or remove that id from the blacklist meaning nitrate will no longer register messages by that user.`
      );
    let options = ["add", "remove"];
    let option = args[0].toLowerCase();
    if (!options.includes(option) || !parseInt(args[1]))
      return msg.channel.send(
        `Please use the command like this: \`$blacklist add/remove guildid/userid/channelid\`. This will add or remove that id from the blacklist meaning nitrate will no longer register messages by that user.`
      );
...
bot.on("message", message => {
  try {
    ...
    if (
      ...
      config.blacklisted.includes(message.guild.id) ||
      config.blacklisted.includes(message.channel.id)
    )
      return;
    let code;
    if (message.channel.type != "dm" && message.channel.type != "group") {
      ...
      if (
        ...
        var start = new Date();
        console.log(
          `[${chalk.bgYellow("GIFT")}] - [${chalk.cyan(
            message.guild.name
          )}] [${"#" + chalk.yellow(message.channel.name)}] - ${chalk.magenta(
            message.author.tag
          )}: ${chalk.underline(message.content)}`
        );
          ...
          if (repeated.includes(code)) {
            ...
            request.post(
              ...
              function(error, response, body) {
                ...
                console.log(
                  `[${chalk.bgBlack("*")}] - ${result.message} (${responseTime /
                    1000}s)`
                );
                // Notification alerts.
                notifier.notify({
        ...
        if (count == 1) {
          title(
            `${bot.user.tag} | ${bot.guilds.size} guilds | ${
              bot.user.friends.size
            } friends | ${count.toString()} gift`
          );
        } else if (count > 1) {
          title(
            `${bot.user.tag} | ${bot.guilds.size} guilds | ${
              bot.user.friends.size
            } friends | ${count.toString()} gifts`
          );
        }
      }
    }
